...
typedef struct gatt_param_t {
    ...
    gatt_write_status_cb_t write_status_cb;
    ...
    struct ble_gatt_access_ctxt* ctxt, void* arg)
{
    ...
    else if (ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR)
    {
        ...
        if (len <= param->value_maxlen)
        {
            ...
            param->value_len = len;
            if (param->write_status_cb)
            {
                return param->write_status_cb(
                    param, param->value_buf, param->value_len);
            }
            if (param->write_cb)
            {
...
int gatt_svr_init(void)
{
    ...
    if (rc != 0)
    {
        printf("%s: Error adding services: %d\n", __func__, rc);
        return rc;
    } 
